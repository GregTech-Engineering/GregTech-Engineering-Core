...
public interface GTEMachines {
    ...
    // *** Simple Machine ***//
    //////////////////////////////////////
    // Pair<MachineDefinition, MachineDefinition> STEAM_VACUUM_PUMP = registerSteamMachines("steam_vacuum_pump", "真空泵",
    // SteamVacuumPumpMachine::new, (pressure, builder) -> builder
    // .allRotation()
    ...
    //
    // MachineDefinition[] ROCKET_ENGINE_GENERATOR = registerSimpleGenerator("rocket_engine", "火箭引擎",
    // GTERecipeTypes.ROCKET_ENGINE_FUELS,
    // GTMachineUtils.genericGeneratorTankSizeFunction, EV, IV, LuV);
    //
    // MachineDefinition[] NAQUADAH_REACTOR_GENERATOR = registerSimpleGenerator("naquadah_reactor", "硅岩反应堆",
    // GTERecipeTypes.NAQUADAH_REACTOR,
    // GTMachineUtils.genericGeneratorTankSizeFunction, IV, LuV, ZPM);

    // MachineDefinition[] ARC_GENERATOR = registerSimpleMachines("arc_generator", "电弧发生器",
    // GTERecipeTypes.ARC_GENERATOR_RECIPES, GTMachineUtils.defaultTankSizeFunction);

    // MachineDefinition[] DEHYDRATOR = registerSimpleMachines("dehydrator", "脱水机", GTERecipeTypes.DEHYDRATOR_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction);
    //
    // MachineDefinition[] UNPACKER = registerSimpleMachines("unpacker", "解包机", GTERecipeTypes.UNPACKER_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction);
    //
    // MachineDefinition[] CLUSTER = registerSimpleMachines("cluster", "多辊式轧机", GTERecipeTypes.CLUSTER_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction);
    //
    // MachineDefinition[] ROLLING = registerSimpleMachines("rolling", "辊轧机", GTERecipeTypes.ROLLING_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction);
    //
    // MachineDefinition[] LAMINATOR = registerSimpleMachines("laminator", "过胶机", GTERecipeTypes.LAMINATOR_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction);
    //
    // MachineDefinition[] LOOM = registerSimpleMachines("loom", "织布机", GTERecipeTypes.LOOM_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction);
    //
    // MachineDefinition[] LASER_WELDER = registerSimpleMachines("laser_welder", "激光焊接器",
    // GTERecipeTypes.LASER_WELDER_RECIPES, GTMachineUtils.defaultTankSizeFunction);
    //
    ...
    //
    // MachineDefinition[] ULV_WIREMILL = registerSimpleMachines("wiremill", "线材轧机", GTERecipeTypes.WIREMILL_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction, GTCEu.id("block/machines/wiremill"), ULV);
    // MachineDefinition[] ULV_LATHE = registerSimpleMachines("lathe", "车床", GTERecipeTypes.LATHE_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction, GTCEu.id("block/machines/lathe"), ULV);
    // MachineDefinition[] ULV_FLUID_SOLIDIFIER = registerSimpleMachines("fluid_solidifier", "流体固化器",
    // GTERecipeTypes.FLUID_SOLIDFICATION_RECIPES, GTMachineUtils.defaultTankSizeFunction,
    // GTCEu.id("block/machines/fluid_solidifier"), ULV);
    // MachineDefinition[] ULV_CHEMICAL_REACTOR = registerSimpleMachines("chemical_reactor", "化学反应釜",
    // GTERecipeTypes.CHEMICAL_RECIPES, GTMachineUtils.defaultTankSizeFunction,
    // GTCEu.id("block/machines/chemical_reactor"), ULV);
    // MachineDefinition[] ULV_ASSEMBLER = registerSimpleMachines("assembler", "组装机", GTERecipeTypes.ASSEMBLER_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction, GTCEu.id("block/machines/assembler"), ULV);
    // MachineDefinition[] ULV_PACKER = registerSimpleMachines("packer", "打包机", GTERecipeTypes.PACKER_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction, GTCEu.id("block/machines/packer"), ULV);
    // MachineDefinition[] ULV_UNPACKER = registerSimpleMachines("unpacker", "解包机", GTERecipeTypes.UNPACKER_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction, ULV);
    // MachineDefinition[] ULV_LOOM = registerSimpleMachines("loom", "织布机", GTERecipeTypes.LOOM_RECIPES,
    // GTMachineUtils.defaultTankSizeFunction, ULV);
    //
    // MachineDefinition[] VACUUM_PUMP = registerTieredMachines("vacuum_pump", tier -> "%s真空泵
    // %s".formatted(GTEValues.VLVHCN[tier], VLVT[tier]), VacuumPumpMachine::new,
    // (tier, builder) -> builder
    // .langValue("%s Vacuum Pump %s".formatted(VLVH[tier], VLVT[tier]))
    // .nonYAxisRotation()
    // .editableUI(SimpleTieredMachine.EDITABLE_UI_CREATOR.apply(GTCEu.id("vacuum_pump"),
    // GTERecipeTypes.VACUUM_PUMP_RECIPES))
    ...
    // .tooltips(GTMachineUtils.workableTiered(tier, V[tier], V[tier] << 6, GTERecipeTypes.VACUUM_PUMP_RECIPES,
    ...
    // .tier(ULV)
    // .editableUI(SimpleNoEnergyMachine.EDITABLE_UI_CREATOR.apply(GTCEu.id("heater"),
    // GTRecipeTypes.STEAM_BOILER_RECIPES))
    ...
    // .tier(ULV)
    // .editableUI(SimpleNoEnergyMachine.EDITABLE_UI_CREATOR.apply(GTCEu.id("boiler"),
    ...
    // .nonYAxisRotation()
    // .tooltipsText("Can monitor the average latency of all machines within 2 seconds and support highlighting.",
    // "能监测全部机器2秒内的平均延迟，并支持高亮显示。")
    ...
    // .tier(LV)
    // .editableUI(SimpleNoEnergyMachine.EDITABLE_UI_CREATOR.apply(GTCEu.id("electric_heater"),
    // GTERecipeTypes.MANA_HEATER_RECIPES))
    ...
    //
    // MachineDefinition[] ACCELERATE_HATCH = registerTieredMachines("accelerate_hatch", tier -> GTEValues.VNFR[tier] +
    // "加速仓",
    ...
    // tiersBetween(LV, MAX));
    //
    // MachineDefinition[] PROGRAMMABLEC_HATCH = registerTieredMachines("programmablec_hatch", tier ->
    // GTEValues.VNFR[tier] + "可编程仓",
    ...
    // Component.translatable("gtceu.universal.tooltip.fluid_storage_capacity_mult", tier,
    // DualHatchPartMachine.getTankCapacity(DualHatchPartMachine.INITIAL_TANK_CAPACITY, tier)),
    // Component.translatable("gtceu.universal.enabled"))
    // .register(),
    // tiersBetween(LV, MAX));
    //
    // MachineDefinition[] ENERGY_INPUT_HATCH_4A = registerTieredMachines("energy_input_hatch_4a", tier -> 4 + "安" +
    // GTEValues.VNFR[tier] + "能源仓",
    ...
    // .renderer(() -> new OverlayTieredMachineRenderer(tier, GTCEu.id("block/machine/part/" +
    // "energy_hatch.input_4a")))
    // .register(),
    // tiersBetween(LV, HV));
    //
    // MachineDefinition[] ENERGY_OUTPUT_HATCH_4A = registerTieredMachines("energy_output_hatch_4a", tier -> 4 + "安" +
    // GTEValues.VNFR[tier] + "动力仓",
    ...
    // .renderer(() -> new OverlayTieredMachineRenderer(tier, GTCEu.id("block/machine/part/" +
    // "energy_hatch.output_4a")))
    // .register(),
    // tiersBetween(LV, HV));
    //
    // MachineDefinition[] ENERGY_INPUT_HATCH_16A = registerTieredMachines("energy_input_hatch_16a", tier -> 16 + "安" +
    // GTEValues.VNFR[tier] + "能源仓",
    ...
    // .tooltips(Component.translatable("gtceu.machine.energy_hatch.input_hi_amp.tooltip"))
    // .renderer(() -> new OverlayTieredMachineRenderer(tier, GTCEu.id("block/machine/part/" +
    // "energy_hatch.input_16a")))
    // .register(),
    // tiersBetween(LV, HV));
    //
    // MachineDefinition[] ENERGY_OUTPUT_HATCH_16A = registerTieredMachines("energy_output_hatch_16a", tier -> 16 + "安"
    // + GTEValues.VNFR[tier] + "动力仓",
    ...
    // PartAbility.OUTPUT_ENERGY);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_256 = registerWirelessLaserHatch(IO.IN, 256,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_256 = registerWirelessLaserHatch(IO.OUT, 256,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_1024 = registerWirelessLaserHatch(IO.IN, 1024,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_1024 = registerWirelessLaserHatch(IO.OUT, 1024,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_4096 = registerWirelessLaserHatch(IO.IN, 4096,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_4096 = registerWirelessLaserHatch(IO.OUT, 4096,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_16384 = registerWirelessLaserHatch(IO.IN, 16384,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_16384 = registerWirelessLaserHatch(IO.OUT, 16384,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_65536 = registerWirelessLaserHatch(IO.IN, 65536,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_65536 = registerWirelessLaserHatch(IO.OUT, 65536,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_262144 = registerWirelessLaserHatch(IO.IN, 262144,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_262144 = registerWirelessLaserHatch(IO.OUT, 262144,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_1048576 = registerWirelessLaserHatch(IO.IN, 1048576,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_1048576 = registerWirelessLaserHatch(IO.OUT, 1048576,
    // PartAbility.OUTPUT_LASER);
    // MachineDefinition[] WIRELESS_INPUT_HATCH_4194304 = registerWirelessLaserHatch(IO.IN, 4194304,
    // PartAbility.INPUT_LASER);
    // MachineDefinition[] WIRELESS_OUTPUT_HATCH_4194304 = registerWirelessLaserHatch(IO.OUT, 4194304,
    // PartAbility.OUTPUT_LASER);
    //
    ...
    // PartAbility.OUTPUT_LASER);
    //
    // MachineDefinition[] NEUTRON_ACCELERATOR = registerTieredMachines("neutron_accelerator", tier -> VNF[tier] +
    // "中子加速器",
    ...
    //
    // MachineDefinition WIRELESS_DATA_HATCH_TRANSMITTER = machine("wireless_data_transmitter_hatch", "无线光学数据源仓",
    // (holder) -> new WirelessOpticalDataHatchMachine(holder, true))
    // .allRotation()
    ...
    // Component.translatable("gtecore.machine.wireless_data_transmitter_hatch.tooltip.0"))
    // .tooltipsKey("gtceu.universal.disabled")
    // .tier(UV)
    // .register();
    //
    // MachineDefinition WIRELESS_DATA_HATCH_RECEIVER = machine("wireless_data_receiver_hatch", "无线光学数据靶仓", (holder) ->
    // new WirelessOpticalDataHatchMachine(holder, false))
    // .allRotation()
    // .abilities(PartAbility.OPTICAL_DATA_RECEPTION)
    // .renderer(() -> new OverlayTieredMachineRenderer(UV, GTCEu.id("block/machine/part/optical_data_hatch")))
    // .tooltips(Component.translatable("gtceu.machine.data_receiver_hatch.tooltip"),
    // Component.translatable("gtecore.machine.wireless_data_receiver_hatch.tooltip.0"))
    // .tooltipsKey("gtceu.universal.disabled")
    // .tier(UV)
    // .register();
    //
    // MachineDefinition LARGE_STEAM_HATCH = machine("large_steam_input_hatch", "大型蒸汽输入仓",
    // LargeSteamHatchPartMachine::new)
    // .allRotation()
    ...
    // .register();
    //
    // MachineDefinition STERILE_CLEANING_MAINTENANCE_HATCH = machine("sterile_cleaning_maintenance_hatch", "无菌维护仓",
    // holder -> new CMHatchPartMachine(holder, CMHatchPartMachine.STERILE_DUMMY_CLEANROOM))
    // .allRotation()
    ...
    // GTECore.id("block/machine/part/maintenance.sterile_cleaning")))
    // .register();
    //
    // MachineDefinition LAW_CLEANING_MAINTENANCE_HATCH = machine("law_cleaning_maintenance_hatch", "绝对洁净维护仓", holder ->
    // new CMHatchPartMachine(holder, CMHatchPartMachine.LAW_DUMMY_CLEANROOM))
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtceu.universal.disabled"),
    // Component.translatable("gtceu.machine.maintenance_hatch_cleanroom_auto.tooltip.0"),
    ...
    // .register();
    //
    // MachineDefinition AUTO_CONFIGURATION_MAINTENANCE_HATCH = machine("auto_configuration_maintenance_hatch",
    // "可配置自动维护仓", ACMHatchPartMachine::new)
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtceu.universal.disabled"))
    ...
    // .register();
    //
    // MachineDefinition CLEANING_CONFIGURATION_MAINTENANCE_HATCH = machine("cleaning_configuration_maintenance_hatch",
    // "超净可配置维护仓", holder -> new CCMHatchPartMachine(holder, CMHatchPartMachine.DUMMY_CLEANROOM))
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtceu.universal.disabled"),
    // Component.translatable("gtceu.machine.maintenance_hatch_cleanroom_auto.tooltip.0"),
    ...
    // .register();
    //
    // MachineDefinition STERILE_CONFIGURATION_CLEANING_MAINTENANCE_HATCH =
    // machine("sterile_configuration_cleaning_maintenance_hatch", "无菌可配置维护仓", holder -> new CCMHatchPartMachine(holder,
    // CMHatchPartMachine.STERILE_DUMMY_CLEANROOM))
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtceu.universal.disabled"),
    // Component.translatable("gtceu.machine.maintenance_hatch_cleanroom_auto.tooltip.0"),
    ...
    // .register();
    //
    // MachineDefinition LAW_CONFIGURATION_CLEANING_MAINTENANCE_HATCH =
    // machine("law_configuration_cleaning_maintenance_hatch", "绝对洁净可配置维护仓", holder -> new CCMHatchPartMachine(holder,
    // CMHatchPartMachine.LAW_DUMMY_CLEANROOM))
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtceu.universal.disabled"),
    // Component.translatable("gtceu.machine.maintenance_hatch_cleanroom_auto.tooltip.0"),
    ...
    // .register();
    //
    // MachineDefinition GRAVITY_CONFIGURATION_HATCH = machine("gravity_configuration_hatch", "可配置重力维护仓",
    // CGCHatchPartMachine::new)
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtceu.universal.disabled"))
    ...
    // .register();
    //
    // MachineDefinition VACUUM_CONFIGURATION_HATCH = machine("vacuum_configuration_hatch", "可配置真空维护仓",
    // CVCHatchPartMachine::new)
    // .allRotation()
    // .abilities(PartAbility.MAINTENANCE)
    // .tooltips(Component.translatable("gtecore.recipe.vacuum.tier", 4))
    // .tooltips(Component.translatable("gtceu.universal.disabled"))
    ...
    // .register();
    //
    // MachineDefinition ROTOR_HATCH = machine("rotor_hatch", "转子仓", h -> new ItemHatchPartMachine(h, 1, i ->
    // TurbineRotorBehaviour.getBehaviour(i) != null))
    // .tier(EV)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    ...
    // .register();
    //
    // MachineDefinition PRIMITIVE_BLAST_FURNACE_HATCH = machine("primitive_blast_furnace_hatch", "土高炉仓",
    // PrimitiveBlastFurnaceHatch::new)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    ...
    // .register();
    //
    // MachineDefinition LENS_HOUSING = machine("lens_housing", "透镜仓", h -> new ItemHatchPartMachine(h, 1, i ->
    // ChemicalHelper.getPrefix(i.getItem()) == TagPrefix.lens))
    // .tier(EV)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    ...
    // .register();
    //
    // MachineDefinition DEGASSING_CONTROL_HATCH = machine("degassing_control_hatch", "脱气控制仓",
    // IndicatorHatchPartMachine::new)
    // .tier(LuV)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    // .overlayTieredHullRenderer("neutron_sensor")
    ...
    // .register();
    //
    // MachineDefinition MACHINE_ACCESS_INTERFACE = machine("machine_access_interface", "机器访问接口",
    // MachineAccessInterfacePartMachine::new)
    // .tier(IV)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    ...
    // .register();
    //
    // MachineDefinition MACHINE_ACCESS_TERMINAL = machine("machine_access_terminal", "机器访问终端",
    // MachineAccessTerminalPartMachine::new)
    // .tier(UEV)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    ...
    // .register();
    //
    // MachineDefinition THERMAL_CONDUCTOR_HATCH = machine("thermal_conductor_hatch", "导热剂仓",
    // ThermalConductorHatchPartMachine::new)
    // .tier(LuV)
    // .allRotation()
    // .tooltipsKey("gtceu.universal.disabled")
    // .overlayTieredHullRenderer("neutron_sensor")
    // .register();
    //
    // MachineDefinition TAG_FILTER_ME_STOCK_BUS_PART_MACHINE = machine("me_tag_filter_stock_bus_part_machine",
    // "ME标签过滤库存输入总线", METagFilterStockBusPartMachine::new)
    // .tier(LuV)
    // .abilities(PartAbility.IMPORT_ITEMS)
    // .allRotation()
    ...
    // .register();
    //
    // MachineDefinition ME_DUAL_HATCH_STOCK_PART_MACHINE = machine("me_dual_hatch_stock_part_machine", "ME库存输入总成",
    // MEDualHatchStockPartMachine::new)
    // .tier(LuV)
    ...
    // .register();
    //
    // MachineDefinition ME_CRAFT_PATTERN_PART_MACHINE = machine("me_craft_pattern_part_machine", "合成样板仓",
    // MECraftPatternPartMachine::new)
    ...
    // .register();
    //
    // MachineDefinition ME_PATTERN_BUFFER = machine("me_catalyst_pattern_buffer", "ME催化剂样板总成",
    // MECatalystPatternBufferPartMachine::new)
    ...
    // .register();
    //
    // MachineDefinition ME_PROGRAMMABLE_PATTERN_BUFFER = machine("me_programmable_pattern_buffer", "ME可编程样板总成",
    // MEProgrammablePatternBufferPartMachine::new)
    ...
    // .register();
    //
    // MachineDefinition INFINITE_PARALLEL_HATCH = machine("infinite_parallel_hatch", "无限并行仓", h -> new
    // ParallelHatchPartMachine(h, -1))
    // .tier(MAX)
    // .allRotation()
    ...
    // .register();
    //
    // MachineDefinition INFINITE_WATER_HATCH = machine("infinite_water_hatch", "无限水仓",
    // InfiniteWaterHatchPartMachine::new)
    // .tier(IV)
    // .allRotation()
    ...
    // .register();
    //
    // MachineDefinition INFINITE_INTAKE_HATCH = machine("infinite_intake_hatch", "无限进气仓",
    // InfiniteIntakeHatchPartMachine::new)
    // .tier(ULV)
    // .allRotation()
    // .abilities(PartAbility.IMPORT_FLUIDS)
    // .tooltipsKey("gtceu.universal.enabled")
